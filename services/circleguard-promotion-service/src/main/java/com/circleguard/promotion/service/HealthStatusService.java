...
public class HealthStatusService {
    ...
    public void updateStatus(String anonymousId, String status, boolean adminOverride) {
        ...
        // Story 4.5: Immediate Redis synchronization for Gate validation
        try {
            redisTemplate.opsForValue().set(STATUS_KEY_PREFIX + anonymousId, status);
            log.debug("Pre-synchronized Redis for user {}: {}", anonymousId, status);
        } catch (Exception e) {
            log.warn("Failed to pre-synchronize Redis for user {}. Continuing...", anonymousId, e);
        }
        ...
        // Simplified high-performance propagation
        String unifiedQuery = 
            ...
            "WHERE (type(r1)='ENCOUNTERED' AND coalesce(r1.isValid, true) AND r1.startTime > $threshold) " +
            "   OR (type(r1)='MEMBER_OF' AND EXISTS { MATCH (source)-[:MEMBER_OF]->(circ:Circle)<-[:MEMBER_OF]-(c1) WHERE coalesce(circ.isValid, true) }) " +
            "WITH source, collect(DISTINCT c1) as l1Nodes " +
            "FOREACH (n IN l1Nodes | " +
            "  SET n.status = CASE WHEN $status = 'CONFIRMED' THEN 'SUSPECT' " +
            "                      WHEN $status = 'SUSPECT' THEN 'PROBABLE' " +
            "                      ELSE n.status END, " +
            "      n.statusUpdatedAt = timestamp() " +
            ") " +
            "WITH source, l1Nodes " +
            "OPTIONAL MATCH (source)-[r1]-(c1_node:User)-[r2]-(c2:User) " +
            "WHERE $status = 'CONFIRMED' " +
            "  AND (type(r1)='ENCOUNTERED' OR type(r1)='MEMBER_OF') " +
            "  AND (type(r2)='ENCOUNTERED' OR type(r2)='MEMBER_OF') " +
            "  AND c2.status = 'ACTIVE' AND c2.anonymousId <> source.anonymousId " +
            "WITH source, l1Nodes, collect(DISTINCT c2) as l2Nodes " +
            "FOREACH (n IN l2Nodes | SET n.status = 'PROBABLE', n.statusUpdatedAt = timestamp()) " +
            "RETURN source.anonymousId as sourceId, " +
            "       [n IN l1Nodes | {id: n.anonymousId, status: n.status}] + " +
            "       [n IN l2Nodes | {id: n.anonymousId, status: n.status}] as affectedContacts";
        ...
        // Always update Redis for the source user
        Map<String, String> cacheUpdates = new HashMap<>();
        cacheUpdates.put(STATUS_KEY_PREFIX + anonymousId, status);

        List<Map<String, Object>> affected = null;
        if (result.isPresent()) {
            affected = (List<Map<String, Object>>) result.get().get("affectedContacts");
            if (affected != null) {
                affected.forEach(m -> {
                    if (m != null && m.get("id") != null) {
                        cacheUpdates.put(STATUS_KEY_PREFIX + m.get("id"), (String) m.get("status"));
                    }
                });
            }
        }

        log.info("Updating Redis with {} entries", cacheUpdates.size());
    ...
    private void updateRedisInBatches(Map<String, String> updates) {
        ...
        log.info("Performing Redis batch update for {} keys", updates.size());
